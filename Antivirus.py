...
class ScanThread(QThread):
    ...
    def run(self):
        ...
        def is_hash_malicious(h):
            try:
                conn = sqlite3.connect('signatures.db')
                c = conn.cursor()
                c.execute("SELECT malware_name, threat_level FROM signatures WHERE sha256_hash = ?", (h,))
                res = c.fetchone()
                conn.close()
                return res
            except:
...
def scan_full_system(self):
    ...
    self.LoadingPageTitle.setText("Scanning full system...")
    QApplication.processEvents()

    self.thread = ScanThread(os.path.expanduser("~/Downloads"))
    self.thread.finished.connect(lambda result: handle_scan_result(self, result))
    self.thread.start()

from PyQt5.QtWidgets import QApplication
def handle_scan_result(self, infected):
    print("✅ Scan finished")
    if infected:
        msg = "\n".join([f"{p} → {n} ({l})" for p, n, l in infected])
        QMessageBox.information(None, "Malware Found", msg)
    else:
        QMessageBox.information(None, "Clean", "No malware found.")
...
def scan_full_system(self):
    ...
    self.thread = ScanThread(root_dir)
    self.thread.finished.connect(lambda result: handle_scan_result(self, result))
    self.thread.start()
    ...
    def is_hash_malicious(file_hash):
        try:
            conn = sqlite3.connect('signatures.db')
            cursor = conn.cursor()
            cursor.execute("SELECT malware_name, threat_level FROM signatures WHERE sha256_hash = ?", (file_hash,))
    ...
    for root, dirs, files in os.walk(root_dir):
        for file in files:
            try:
                ...
                result = is_hash_malicious(file_hash)
                if result:
                    infected.append((full_path, result[0], result[1]))
            except:
                continue

    if infected:
        msg = "\n".join([f"{p} → {name} ({level})" for p, name, level in infected])
        QtWidgets.QMessageBox.information(None, "Malware Found", msg)
    else:
        QtWidgets.QMessageBox.information(None, "Clean", "No malware found.")
    ...
    def run(self):
        ...
        for root, dirs, files in os.walk(self.root_dir):
            for file in files:
                try:
                    path = os.path.join(root, file)
                    if os.path.getsize(path) > 50 * 1024 * 1024:  # skip big files
                        continue
                    with open(path, 'rb') as f:
                        h = hashlib.sha256(f.read()).hexdigest()
                    result = is_hash_malicious(h)
